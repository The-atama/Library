...
namespace Util {
template <class T> vector<pair<T, int>> runLength(vector<T> v) {
...
vector<vector<int>> square(vector<string> &s, char c) {
  ...
  for (int i = n - 1; i >= 0; i--) {
    for (int j = m - 1; j >= 0; j--) {
      if (s[i][j] == c)
        ru[i][j] = min(min(ru[i + 1][j + 1], r[i][j + 1]), u[i + 1][j]) + 1;
      else
        ru[i][j] = 0;
    }
  }
...
template <class D> struct SparseTable2D {
  ...
  SparseTable2D(const vector<vector<D>> &data, const DMerger dm) : dm(dm) {
    ...
    for (int k = 1; k <= logm; k++) {
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
          if (j + (1 << (k - 1)) < m) {
            table[i][0][j][k] = dm(table[i][0][j][k - 1],
                                   table[i][0][j + (1 << (k - 1))][k - 1]);
          } else {
    ...
    for (int k = 1; k <= logn; k++) {
      for (int l = 0; l <= logm; l++) {
        for (int i = 0; i < n; i++) {
          for (int j = 0; j < m; j++) {
            if (i + (1 << (k - 1)) < n) {
              table[i][k][j][l] = dm(table[i][k - 1][j][l],
                                     table[i + (1 << (k - 1))][k - 1][j][l]);
            } else {
  ...
  D query(int x1, int y1, int x2, int y2) {
    ...
    D tmp2 = dm(table[x1][xd][y2 - (1 << yd)][yd],
                table[x2 - (1 << xd)][xd][y2 - (1 << yd)][yd]);
    return dm(tmp1, tmp2);
  }
};
...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
      int d = nano[i][j];
      if (d > 0)
        chmax(logo[i + d - 1][j + d - 1], nano[i][j]);
    }
  }
  ...
  for (int i = 0; i < q; i++) {
    ...
    while (r - l > 1) {
      int mid = (l + r) / 2;
      if (st.query(r1 + mid - 1, c1 + mid - 1, r2 - mid + 1, c2 - mid + 1) >=
          mid)
        l = mid;
      else
        r = mid;
