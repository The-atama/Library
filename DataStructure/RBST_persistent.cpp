...
struct RBST {
  ...
  Node *alloc(val_t val) { return &(pool[memory_size++] = Node(val)); }
  Node *copy(Node *x) { return &(pool[memory_size++] = Node(x)); }
  ...
  void copypaste(int a, int b, int c) { // 0-indexed [a,b)
    ...
    if (size(root) > length_limit)
      root = split(root, length_limit).fi;
    return;
  }
  void write(Node *x) {
    if (x == NULL)
...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (tree.memory_size > memory_limit)
      tree.rebuild();
  }
  tree.rebuild();
  printf("%s\n", tree.str);
