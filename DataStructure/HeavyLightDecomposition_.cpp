...
struct HeavyLightDecomposition {
  ...
  void dfs() {
    ...
    while (pos) {
      ...
      if (now.go) {
        depth[v] = d;
        par[v] = p;
        st[pos++] = state(v, p, d, false);
        for (int i = 0; i < g[v].size(); i++) {
          if (g[v][i] == p)
            continue;
          st[pos++] = state(g[v][i], v, d + 1, true);
        }
      } else {
        chnum[v] = 1;
        for (int i = 0; i < g[v].size(); i++) {
    ...
    while (pos) {
      ...
      for (int i = 0; i < g[v].size(); i++) {
        if (g[v][i] == p)
          continue;
        if (g[v][i] == hev)
          st[pos++] = state(g[v][i], v, false);
        else
          st[pos++] = state(g[v][i], v, true);
      }
    }
  }
  ...
  ll solve(int v) {
    ...
    while (now != -1) {
      ...
      if (now != -1)
        ret +=
            seg.query(toseg[now], toseg[now] + 1) * (ll)(depth[v] - depth[now]);
    }
    return ret;
  }
