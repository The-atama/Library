...
void Update(int kx, int ky, int x) {
  ...
  while (1) {
    ...
    kx = (kx - 1) / 2;
    Segtree[kx].seg[ky] =
        Segtree[kx * 2 + 1].seg[ky] + Segtree[kx * 2 + 2].seg[ky];
  }
  return;
}
ll Query(int a, int b, int c, int d, int k, int l,
         int r) // note:row row column column
{
  if (r <= a || b <= l)
    return 0;
  else if (a <= l && r <= b)
    ...
    return Query(a, b, c, d, k * 2 + 1, l, (l + r) / 2) +
