...
//               id : initially add line "0*x+id"
//               (LLINF,numeric_limits<ll>::max() etc.[in case of MIN])
// member @
//           add_line    : add line "ax+b"              in O(logN) time
//           add_segment : add segment "ax+b" to [l,r)  in O((logN)^2) time
//

template <class T, bool Comptype = true> struct LiChaoTree {
  using Line = pair<T, T>;
  int sz = 1;
  vector<Line> seg;
  vector<T> xs;
  ...
  static inline T f(Line l, ll x) { return l.fi * x + l.sec; }
  ...
  void add_line(Line line) { add_line(line, 0, 0, sz); }
