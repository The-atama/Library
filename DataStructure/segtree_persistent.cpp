...
node *add(node *v, int pos, int l, int r) {
  if (r - l == 1)
    return mynew(nil, nil, 1);
  int mid = (l + r) / 2;
  if (pos < mid)
    return mynew(add(v->lch, pos, l, mid), v->rch);
  else
    return mynew(v->lch, add(v->rch, pos, mid, r));
}
int mx, tmp;
...
int main() {
  ...
  for (int i = 0; i < N; i++)
    h[i] = lower_bound(all(vs), h[i]) - vs.begin();
  for (int i = 0; i < N; i++)
    xs[h[i]].pb(i);
  int num = vs.size();
  for (int i = 0; i < num; i++) {
    root[i + 1] = root[i];
    for (int j = 0; j < xs[num - i - 1].size(); j++)
      root[i + 1] = add(root[i + 1], xs[num - i - 1][j], 0, N);
  }
  scanf("%d", &m);
  /*while(1){
          int m,l,r;
