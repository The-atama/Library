...
void yosupo_RangeChminChmaxAddRangeSum() {
  ...
  for (int i = 0; i < q; i++) {
    ...
    if (type == 3) {
      cin >> l >> r;
      cout << seg.query(l, r, &Data::get_sum, [](ll x, ll y) { return x + y; },
                        0ll)
           << endl;
    }
    /*
  ...
  for (int T = 0; T < t; T++) {
    ...
    for (int i = 0; i < m; i++) {
      ...
      if (type == 1) {
        ...
        x--;
        printf("%lld\n",
               seg.query(x, y, &Data::get_max,
                         [](ll x, ll y) { return max(x, y); }, -LLINF));
      }
      if (type == 2) {
        // cin >> x >> y;
        scanf("%d %d", &x, &y);
        x--;
        printf("%lld\n", seg.query(x, y, &Data::get_sum,
...
void AOJ_DSL_2_F() {
  ...
  for (int i = 0; i < q; i++) {
    ...
    if (type == 0) {
      cin >> l >> r >> x;
      seg.update(l, r + 1, &Data::chmax_query, 1000000000000ll);
      seg.update(l, r + 1, &Data::chmin_query, x);
    }
    if (type == 1) {
      cin >> l >> r;
      cout << seg.query(l, r + 1, &Data::get_min,
                        [](ll x, ll y) { return min(x, y); }, LLINF)
           << endl;
    }
  }
}
...
void AOJ_DSL_2_I() {
  ...
  for (int i = 0; i < q; i++) {
    ...
    if (type == 1) {
      cin >> l >> r;
      cout << seg.query(l, r + 1, &Data::get_sum,
                        [](ll x, ll y) { return x + y; }, 0ll)
           << endl;
    }
  }
